...
- **재생 완료 시 출석 검증 결과를 이벤트 로그에 기록**: 자동 재생이 완료로 처리될 때
  `progress_reported`(진도 API 보고 수락 여부) / `lms_progress_ratio`(재조회한 LMS 저장 진도)를
  `player/play_complete` 이벤트 metadata에 남긴다 (`backend/api/routes/auto.py`). 기존엔
  성공 케이스는 아무 기록도 없어 "출석이 실제로 반영됐는지" 사후 확인이 불가능했다. 이제
  로그 화면·이벤트 로그에서 강의별 반영 여부를 바로 확인할 수 있다.
